...
03/25/2026 Hammad Khan
Here we have added the hit and effort spike trials together which should be referenced on the final rslds
spike cells. Within that same structure we can grab the lever traces. 
...
def train_rslds_from_mat(mat_file_path, save_path):
    with h5py.File(mat_file_path, 'r') as f:
        ...
        lever_traces = perturbbehav['leverTraces'][:]
        print("\n--- Extracted Pull Data ---")  
        print(f"Shape of leverTraces: {lever_traces.shape}")
        ...
        if rslds_spk:
            print("\n--- Extracted Data ---")
            print("Number of trial cells:", len(rslds_spk))
    ...
    print(f"Shape of lever trace array: {lever_traces.shape}")
    assert lever_traces.shape[0] == n_trials, "Number of trials in the lever trace does not match number of trials in spk_ref"
    #%%
    spike_data = spk_ref.reshape(n_time*n_trials,n_neurons)
    lever_data = np.reshape(lever_traces, lever_traces.shape[0]*lever_traces.shape[1])
    ...
    print(f"Generated lever data shape: {lever_data.shape}")

    from motorCortexLever.spike_utilities import compute_binned_spike_data, bin_lever_trace
    # Parameters
    ...
    binned_lever_data = bin_lever_trace(lever_data, bin_size_ms=bin_size_ms)
    ...
    # reshape input to have 1 as the second dimension for the input driven model
    binned_lever_data = binned_lever_data.reshape(-1, 1)
    slds = SLDS(obs_dim, num_states, latent_dim, emissions="poisson_orthog", transitions="recurrent",emission_kwargs=dict(link="softplus"),M=input_dim)
    binned_spike_data = binned_spike_data.astype(np.int32)
    assert binned_spike_data.dtype == int
    # assert that binned spike data is equal to binned lever trace in terms of time dimension
    assert binned_spike_data.shape[0] == binned_lever_data.shape[0], "Time dimension of binned spike data and binned lever trace do not match"
    slds.initialize(binned_spike_data,binned_lever_data,verbose=1)
    # Fit the model using Laplace-EM with a structured variational posterior
    q_lem_elbos, q_lem = slds.fit(binned_spike_data,binned_lever_data, method="laplace_em",
...
def batch_train_rslds(directory_path):
    ...
    models_dir = os.path.join(directory_path, 'rslds_models_new_inputdriven')
