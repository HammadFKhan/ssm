...
mat_file_path = r"D:\BayesianWaveModel\ShuffledWaves\notagSOM_Day1M2Waves_unshuffled.mat"
...
with h5py.File(mat_file_path, 'r') as f:
    ...
    hit = intan_behaviour['MIHitTrace']
...
dy_mean = dy_array.mean(axis=(2, 3))
X_mean = np.stack([dx_mean, dy_mean], axis=-1).astype(np.float32)
X_mean = np.stack([dx_array[:, :, 0, 0], dy_array[:, :, 0, 0]], axis=-1)
Y = lever_traces[..., None].astype(np.float32)            # (n_trials, T, 1)
...
# Load shuffled wave data using the same procedure as unshuffled data
mat_file_path = r"D:\BayesianWaveModel\ShuffledWaves\notagSOM_Day1M2Waves_shuffled.mat"
...
with h5py.File(mat_file_path, 'r') as f:
    print(list(f.keys()))
    intan_behaviour = f['WavesHit']

    # ---- lever traces (hit trials) ----
    hit = intan_behaviour
    hit_trace = hit['levertrace']          # object references to each trial
    hit_refs = hit_trace[:].flatten()
    lever_traces = [f[ref][:] for ref in hit_refs]
    lever_traces = np.squeeze(np.stack(lever_traces, axis=0))  # (n_trials, T)
    ...
    # Inspect one example to verify original ordering
    print("Example dx_list[0].shape:", dx_list[0].shape)
    print("Example dy_list[0].shape:", dy_list[0].shape)

    # Assuming each dx/dy is (8, 8, T), stack into (n_trials, 8, 8, T)
    dx_array_shuf = np.stack(dx_list, axis=0)
    dy_array_shuf = np.stack(dy_list, axis=0)

    print("dx_array_shuf raw shape:", dx_array_shuf.shape)
    print("dy_array_shuf raw shape:", dy_array_shuf.shape)

# Transpose to match original data format: (n_trials, T, 8, 8)
# Make sure this transpose is identical to the one used for unshuffled data
dx_array_shuf = np.transpose(dx_array_shuf, (0, 1, 3, 2))
dy_array_shuf = np.transpose(dy_array_shuf, (0, 1, 3, 2))

print("dx_array_shuf transposed shape:", dx_array_shuf.shape)
print("dy_array_shuf transposed shape:", dy_array_shuf.shape)

# Basic consistency checks: time dimension should match lever_traces
assert lever_traces.shape[0] == dx_array_shuf.shape[0] == dy_array_shuf.shape[0], \
    "Mismatch in number of trials between lever_traces and dx/dy arrays"
assert lever_traces.shape[1] == dx_array_shuf.shape[1] == dy_array_shuf.shape[1], \
    "Mismatch in time dimension between lever_traces and dx/dy arrays"

# %% Test full TCN model on shuffled wave data

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# (n_trials, T, 8, 8, 2) -> (n_trials, T, 8*8*2)
X_full_shuf = np.stack([dx_array_shuf, dy_array_shuf], axis=-1)  # (n_trials, T, 8, 8, 2)
X_full_shuf = X_full_shuf.reshape(X_full_shuf.shape[0], X_full_shuf.shape[1], -1).astype(np.float32)

Y_shuf = lever_traces[..., None].astype(np.float32)  # (n_trials, T, 1)

# Check shapes versus original X_full and Y_full if available
# assert X_full_shuf.shape[1] == X_full.shape[1], "Time dimension of shuffled and original X_full differ"
# assert Y_shuf.shape[1] == Y_full.shape[1], "Time dimension of shuffled and original Y_full differ"

# Use same train/test split indices as original model
X_full_shuf_te = X_full_shuf[test_idx]
Y_shuf_te = Y_shuf[test_idx]

# Evaluate model_full on shuffled data using original normalization stats
shuf_corrs = []
shuf_rmses = []
...
with torch.no_grad():
    for tr_local, tr in enumerate(test_idx):
        # Normalize using ORIGINAL training stats
        x_s = ((X_full_shuf[tr:tr+1] - X_full_mean) / X_full_std).astype(np.float32)
        x_s = torch.from_numpy(x_s).to(device)

        # If model_full expects (batch, channels, T), uncomment the next line:
        # x_s = x_s.permute(0, 2, 1)

        y_pred_s_norm = model_full(x_s).cpu().numpy()[0, :, 0]
        y_pred_s = y_pred_s_norm * Y_full_std + Y_full_mean

        y_true = lever_traces[tr]

        shuf_corrs.append(np.corrcoef(y_true, y_pred_s)[0, 1])
        shuf_rmses.append(np.sqrt(mean_squared_error(y_true, y_pred_s)))

print("Shuffled TCN mean corr:", np.nanmean(shuf_corrs), "mean RMSE:", np.nanmean(shuf_rmses))

# Compare: Original vs Shuffled
print("\n--- Model Performance Comparison ---")
print(f"Full TCN (original)  corr: {np.nanmean(full_corrs):.4f} ± {np.nanstd(full_corrs, ddof=1)/np.sqrt(len(full_corrs)):.4f}")
print(f"Full TCN (shuffled)  corr: {np.nanmean(shuf_corrs):.4f} ± {np.nanstd(shuf_corrs, ddof=1)/np.sqrt(len(shuf_corrs)):.4f}")

# Plot comparison
fig, ax = plt.subplots(figsize=(4, 4))
means = [np.nanmean(full_corrs), np.nanmean(shuf_corrs)]
sems = [np.nanstd(full_corrs, ddof=1)/np.sqrt(len(full_corrs)),
        np.nanstd(shuf_corrs, ddof=1)/np.sqrt(len(shuf_corrs))]
labels = ['Original', 'Shuffled']
bars = ax.bar(labels, means, yerr=sems, capsize=5, color=['C2', 'C3'], alpha=0.8)
ax.set_ylabel('Test trial correlation')
ax.set_title('Decoder performance shuffled')
ax.set_ylim(0, 1.0)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "decoder_correlations_shuffled.pdf"),
