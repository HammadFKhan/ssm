...
mat_file_path = r"D:\BayesianWaveModel\CooledWaves\69012SomCooling_Day2_BaselineWaves.mat"
...
plt.ylim(0, 1.0)
plt.tight_layout()
plt.savefig('decoder_correlations.pdf',
            format='pdf',
...
# precompute time-resolved saliency
sal_per_t = []
start = 500
for t in tqdm(range(start,T), desc="Computing saliency per timestep"):
    model_full.zero_grad()
    if x.grad is not None:
        x.grad.zero_()
    y_pred[0, t, 0].backward(retain_graph=True)
    g = x.grad.detach().cpu().numpy()[0]      # (T, 128)
    sal_per_t.append(np.abs(g[t]))
sal_per_t = np.stack(sal_per_t, axis=0).reshape(T - start, 8, 8, 2)

# set up figure
fig, axes = plt.subplots(1, 2, figsize=(8, 4))

im_dx = axes[0].imshow(sal_per_t[0, :, :, 0].T,
                       cmap='viridis', origin='upper')
axes[0].set_title('dx saliency')
plt.colorbar(im_dx, ax=axes[0])

im_dy = axes[1].imshow(sal_per_t[0, :, :, 1].T,
                       cmap='viridis', origin='upper')
axes[1].set_title('dy saliency')
plt.colorbar(im_dy, ax=axes[1])

# shared color limits, skipping early frames
skip = 50
vmin = sal_per_t[skip:].min()
vmax = sal_per_t[skip:].max() / 5
im_dx.set_clim(vmin, vmax)
im_dy.set_clim(vmin, vmax)
...
def update(frame_idx):
    t = frame_idx
    im_dx.set_data(sal_per_t[frame_idx, :, :, 0].T)
    im_dy.set_data(sal_per_t[frame_idx, :, :, 1].T)
    time_text.set_text(f'Frame {t} / {T-1}')
    return im_dx,

ani = FuncAnimation(fig, update, frames=T, interval=15, blit=False)

writer = PillowWriter(fps=60)
ani.save(f'saliency_trial{trial}_dxdytest.gif', writer=writer)
plt.close(fig)
...
plt.tight_layout()
plt.savefig('Readout_weights .pdf',
            format='pdf',
            bbox_inches='tight')   # trims extra whitespace
# %%
model_full.eval()
...
plt.savefig('wave_pg_regressor.pdf',
            format='pdf',
            bbox_inches='tight')   # trims extra whitespace
...
for tr in test_idx:
    ...
    sal_t = np.mean(np.abs(sal), axis=0)          # (128,)
    sal_map = sal_t.reshape(8, 8, 2)              # (8, 8, 2)
