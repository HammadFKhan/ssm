...
mat_file_path = r"D:\BayesianWaveModel\CooledWaves\69012SomCooling_Day2_Waves.mat"

def run_decoder_for_condition(f, behav_key, waves_key, tag):
    import h5py
    import numpy as np
    import os
    import torch
    ...
    behav_key: e.g., 'IntanBehaviourBaseline'
    waves_key: e.g., 'WavesBaseline'
    tag: string used in output filenames, e.g. 'Baseline'
    """
     # At the very end, when making output_dir, use tag to separate files:
    mat_basename = os.path.splitext(os.path.basename(mat_file_path))[0]
    output_dir = os.path.join(r"D:\BayesianWaveModel\model_output",
                              f"{mat_basename}_{tag}")
    os.makedirs(output_dir, exist_ok=True)

    intan_behaviour = f[behav_key]

    # ---- lever traces (hit trials) ----
    hit = intan_behaviour  # ['hitTrace']  # if needed
    ...
    print(f"[{tag}] lever_traces shape:", lever_traces.shape)

    # ---- wave struct for hit trials: dx, dy ----
    waves_hit = f[waves_key]
    ...
    print(f"[{tag}] raw dx_array shape:", dx_array.shape)
    print(f"[{tag}] raw dy_array shape:", dy_array.shape)

    # after constructing dx_array, dy_array with shape (n_trials, 3001, 8, 8)
    dx_array = np.transpose(dx_array, (0, 1, 3, 2))
    dy_array = np.transpose(dy_array, (0, 1, 3, 2))

    print(f"[{tag}] transposed dx_array shape:", dx_array.shape)
    ...
    plt.tight_layout()
    plt.savefig(
                os.path.join(output_dir, f"decoder_correlations_{tag}.pdf"),
                format='pdf',
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"lever_decoder_{tag}.pdf"),
                format='pdf',
                bbox_inches='tight')   # trims extra whitespace
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"saliency_maps_{tag}.pdf"),
                format='pdf',
                bbox_inches='tight')   # trims extra whitespace
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"Latent_TCN_{tag}.pdf"),
                format='pdf',
                bbox_inches='tight')   # trims extra whitespace
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"Readout_weights_{tag}.pdf"),
                format='pdf',
                bbox_inches='tight')   # trims extra whitespace
    # 
    model_full.eval()
    ...
    axes[-1].set_xlabel('Time (samples)')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"top_latents_{tag}.pdf"),
                format='pdf',
                bbox_inches='tight')   # trims extra whitespace
    ...
    plt.savefig(os.path.join(output_dir, f"wave_pg_regressor_{tag}.pdf"),
                format='pdf',
                bbox_inches='tight')   # trims extra whitespace
    ...
    # When saving, use the variables from this run, exactly as you already do:
    # np.savez(...), sio.savemat(...), torch.save(...)

    print(f"[{tag}] Results saved to {output_dir}")

        # Save as .npz
    np.savez(
        os.path.join(output_dir, f"tcn_decoder_results_{tag}.npz"),
        test_idx=test_idx,
        train_idx=train_idx,
    ...
    sio.savemat(
        os.path.join(output_dir, f"tcn_decoder_results_{tag}.mat"),
        mat_data
    )

    torch.save(
    model_full.state_dict(),
    os.path.join(output_dir, f"model_full_tcn_{tag}.pt")
    )
    torch.save(
        mean_tcn.state_dict(),
        os.path.join(output_dir, f"model_mean_tcn_{tag}.pt")
    )

# %% Main script to run decoder for different conditions
import h5py
with h5py.File(mat_file_path, 'r') as f:
    print(list(f.keys()))

    results_baseline = run_decoder_for_condition(
        f,
        behav_key='IntanBehaviourBaseline',
        waves_key='WavesBaseline',
        tag='Baseline',
    )

    results_cooled = run_decoder_for_condition(
        f,
        behav_key='IntanBehaviourCooled',
        waves_key='WavesCooled',
        tag='Cooled',
    )

    results_recov = run_decoder_for_condition(
        f,
        behav_key='IntanBehaviourRecov',
        waves_key='WavesRecov',
        tag='Recov',
    )
